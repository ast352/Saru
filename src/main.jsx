...
function Cart({ cart, change, remove, go, user, onOrdered }) {
  ...
  useEffect(()=>{if(user)api.addresses().then(({addresses:items})=>{setAddresses(items);const preferred=items.find(a=>a.is_default)||items[0];if(preferred)setAddressChoice(String(preferred.id))}).catch(err=>setError(err.message))},[user]);
  const order=async e=>{
    e.preventDefault();setBusy(true);setError('');
    try{const data=Object.fromEntries(new FormData(e.currentTarget));const payload=addressChoice==='new'?{name:data.name,phone:data.phone,address:data.address,comment:data.comment}:{addressId:Number(addressChoice),comment:data.comment};const {order:created}=await api.createOrder(payload);setSuccess(created);onOrdered()}
    catch(err){setError(err.message)}finally{setBusy(false)}
  };
  if(success)return <main className="order-success"><div className="success-mark"><Icon name="check" size={34}/></div><span className="micro">Заказ оформлен</span><h1>Спасибо за ваш выбор.</h1><p>Заказ <strong>№{success.id}</strong> принят. Менеджер SARU свяжется с вами для подтверждения доставки.</p><div className="success-order"><header><span>Состав заказа</span><strong>{money(Number(success.total))}</strong></header>{success.items.map(item=><div key={item.id}><span>{item.name}<small>Размер {item.size} · {item.quantity} шт.</small></span><strong>{money(item.price*item.quantity)}</strong></div>)}<footer><span>Адрес доставки</span><p>{success.address}</p></footer></div><div className="success-actions"><button className="action compact" onClick={()=>go('account')}>Перейти в профиль</button><button className="link-arrow" onClick={()=>go('catalog')}>Вернуться к коллекции <Icon name="arrow"/></button></div></main>;
  ...
  return <main className="shell cart"><header><div><span className="micro">Ваш выбор</span><h1>Корзина</h1></div><p>{count} {countLabel}</p></header>
    ...
    <aside className="total"><span className="micro">Ваш заказ</span><h2>Итого</h2><dl><div><dt>Товары · {count}</dt><dd>{money(total)}</dd></div><div><dt>Доставка</dt><dd>Рассчитаем позже</dd></div></dl><div className="total-sum"><span>К оплате</span><strong>{money(total)}</strong></div>
      ...
      {addresses.length>0&&<div className="checkout-addresses">{addresses.map(address=><label className={addressChoice===String(address.id)?'active':''} key={address.id}><input type="radio" name="addressChoice" value={address.id} checked={addressChoice===String(address.id)} onChange={e=>setAddressChoice(e.target.value)}/><span><strong>{address.label}{address.is_default?' · основной':''}</strong><small>{address.city}, {address.street}, {address.house}{address.apartment?`, кв. ${address.apartment}`:''}</small><small>{address.recipient_name} · {address.phone}</small></span></label>)}<label className={addressChoice==='new'?'active':''}><input type="radio" name="addressChoice" value="new" checked={addressChoice==='new'} onChange={e=>setAddressChoice(e.target.value)}/><span><strong>Другой адрес</strong><small>Заполнить данные для этого заказа</small></span></label></div>}
      {addressChoice==='new'&&<div className="checkout-new"><label>Получатель<input name="name" required defaultValue={user?.name}/></label><label>Телефон<input name="phone" required placeholder="+7 900 000-00-00"/></label><label>Адрес<textarea name="address" required placeholder="Город, улица, дом, квартира"/></label></div>}
      <p className="checkout-email"><Icon name="check" size={14}/> Подтверждение будет отправлено на {user?.email}</p>
...
function Account({ user, go, onUser, onLogout }) {
  ...
  const initials=String(user?.name||user?.email||'S').trim().split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();
  const statusName={new:'Новый',confirmed:'Подтверждён',shipped:'Отправлен',completed:'Завершён',cancelled:'Отменён'};
  const word=(count,one,few,many)=>count%10===1&&count%100!==11?one:count%10>=2&&count%10<=4&&(count%100<12||count%100>14)?few:many;
  return <main className="account-page"><header className="account-hero"><div className="account-avatar">{initials}</div><div><span className="micro">Личный кабинет</span><h1>{user?.name || 'Профиль'}</h1><p>{user?.email}</p></div><div className="account-hero-meta"><span><strong>{orders.length}</strong> {word(orders.length,'заказ','заказа','заказов')}</span><span><strong>{addresses.length}</strong> {word(addresses.length,'адрес','адреса','адресов')}</span><span className={user?.emailVerified?'verified':'unverified'}><Icon name={user?.emailVerified?'check':'close'} size={15}/>{user?.emailVerified?'Почта подтверждена':'Почта не подтверждена'}</span></div></header>
    <div className="account-layout"><aside className="account-nav"><span className="micro">Мой профиль</span><nav>{[['overview','Обзор'],['orders','Заказы'],['addresses','Адреса доставки'],['security','Безопасность']].map(([id,label],index)=><button className={view===id?'active':''} onClick={()=>{setView(id);setError('');setMessage('')}} key={id}><i>0{index+1}</i><span>{label}</span>{id==='orders'&&orders.length>0&&<b>{orders.length}</b>}</button>)}</nav><button className="account-catalog" onClick={()=>go('catalog')}>Смотреть коллекцию <Icon name="arrow" size={17}/></button><button className="account-logout" onClick={onLogout}>Выйти из профиля</button></aside>
    <section className="account-content">{error&&<p className="auth-error">{error}</p>}{message&&<p className="profile-message"><Icon name="check" size={16}/>{message}</p>}
      {view==='overview'&&<><header className="account-section-head"><span className="micro">Основное</span><h2>Ваши данные</h2><p>Здесь можно изменить имя и проверить статус электронной почты.</p></header>
        <div className="profile-summary"><article><span>Имя</span><strong>{user?.name}</strong></article><article><span>Электронная почта</span><strong>{user?.email}</strong></article><article><span>Статус</span><strong className={user?.emailVerified?'verified-text':'unverified-text'}>{user?.emailVerified?'Подтверждена':'Ожидает подтверждения'}</strong></article></div>
        {!user?.emailVerified&&<section className="verification-card"><div><Icon name="user" size={25}/><span><strong>Подтвердите почту</strong><small>Ссылка действует 24 часа и помогает защитить профиль.</small></span></div><button className="outline-action" onClick={resend}>Отправить письмо</button>{verificationToken&&<div className="local-verification"><input value={verificationToken} onChange={e=>setVerificationToken(e.target.value)}/><button onClick={verify}>Подтвердить локально</button></div>}</section>}
        <section className="profile-panel personal-card"><div className="panel-title"><span>01</span><div><h3>Личные данные</h3><p>Это имя будет указано при оформлении заказа.</p></div></div><form onSubmit={saveName}><label>Ваше имя<input name="name" defaultValue={user?.name} required maxLength="100"/></label><button className="outline-action">Сохранить изменения</button></form></section></>}
      {view==='orders'&&<><header className="account-section-head"><span className="micro">История покупок</span><h2>Заказы</h2><p>Статусы и состав оформленных заказов.</p></header><section className="order-history">{orders.length?orders.map(o=><article key={o.id}><div className="order-number"><small>{new Date(o.created_at).toLocaleDateString('ru-RU')}</small><strong>Заказ №{o.id}</strong></div><div><span>{o.items.map(i=>`${i.name}, ${i.size} × ${i.quantity}`).join(' · ')}</span><b>{money(Number(o.total))}</b></div><i className={`order-${o.status}`}>{statusName[o.status]}</i></article>):<div className="account-empty"><div><Icon name="bag" size={28}/></div><h3>Заказов пока нет</h3><p>Ваши будущие покупки появятся здесь.</p><button className="link-arrow" onClick={()=>go('catalog')}>Смотреть сорочки <Icon name="arrow"/></button></div>}</section></>}
      {view==='addresses'&&<><header className="account-section-head"><span className="micro">Доставка</span><h2>Адреса</h2><p>Сохраните адрес, чтобы быстрее оформлять следующие заказы.</p></header><div className="address-list">{addresses.map(a=><article key={a.id}><div><Icon name="check" size={16}/>{a.is_default&&<span>Основной</span>}</div><strong>{a.label}</strong><span>{a.recipient_name} · {a.phone}</span><p>{a.city}, {a.street}, {a.house}{a.apartment?`, кв. ${a.apartment}`:''}</p><button onClick={()=>run(async()=>{await api.deleteAddress(a.id);await load()})}>Удалить адрес</button></article>)}</div><section className="profile-panel address-create"><div className="panel-title"><span>+</span><div><h3>Новый адрес</h3><p>Заполните только необходимые данные.</p></div></div><form onSubmit={addAddress} className="address-form"><label>Название<input name="label" placeholder="Например, дом" required/></label><label>Получатель<input name="recipientName" defaultValue={user?.name} required/></label><label>Телефон<input name="phone" placeholder="+7 900 000-00-00" required/></label><label>Город<input name="city" required/></label><label className="wide-field">Улица<input name="street" required/></label><label>Дом<input name="house" required/></label><label>Квартира<input name="apartment"/></label><label>Индекс<input name="postalCode"/></label><label className="check-field"><input type="checkbox" name="isDefault"/> Сделать основным</label><button className="outline-action">Сохранить адрес</button></form></section></>}
      {view==='security'&&<><header className="account-section-head"><span className="micro">Защита профиля</span><h2>Безопасность</h2><p>Обновляйте пароль и управляйте доступом к аккаунту.</p></header><section className="profile-panel security-card"><div className="panel-title"><span>01</span><div><h3>Смена пароля</h3><p>Используйте не менее 10 символов.</p></div></div><form onSubmit={changePassword}><label>Текущий пароль<input name="currentPassword" type="password" autoComplete="current-password" required/></label><label>Новый пароль<input name="newPassword" type="password" autoComplete="new-password" minLength="10" maxLength="128" required/></label><button className="outline-action">Обновить пароль</button></form></section><section className="danger-zone"><span className="micro">Необратимое действие</span><h3>Удаление профиля</h3><p>Личные данные и адреса будут удалены. История заказов останется в обезличенном виде.</p><form onSubmit={removeAccount}><input name="password" type="password" required placeholder="Введите текущий пароль"/><button>Удалить профиль</button></form></section></>}
    </section></div></main>;
}
...
function Admin({ products, setProducts, user, go }) {
  ...
  const status=async(o,value)=>{setError('');try{const {order}=await api.updateOrder(o.id,value);const {products:nextProducts}=await api.products();setOrders(orders.map(x=>x.id===o.id?order:x));setProducts(nextProducts)}catch(err){setError(err.message)}};
  const orderStatuses=[['new','Новый'],['confirmed','Подтверждён'],['shipped','Отправлен'],['completed','Завершён'],['cancelled','Отменён']];
  const orderTransitions={new:['new','confirmed','cancelled'],confirmed:['confirmed','shipped','cancelled'],shipped:['shipped','completed'],completed:['completed'],cancelled:['cancelled']};
  ...
  return <main className="admin"><aside><div><button className="logo" onClick={() => go('home')}>SARU</button><span>Панель модератора</span></div><nav><button className={tab==='products'?'active':''} onClick={()=>setTab('products')}>Товары <b>{products.length}</b></button><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}>Заказы <b>{orders.length}</b></button></nav><div className="admin-user"><small>Модератор</small><span>{user.email}</span></div><button className="admin-exit" onClick={() => go('home')}><Icon name="back"/> На сайт</button></aside><section>
    ...
    <div className="admin-list"><div className="admin-list-head"><span>Карточка</span><span>Остаток</span><span>Цена</span><span>Статус</span><span>Действия</span></div>{products.map(p => <article key={p.id}><div className="admin-thumb">{p.image ? <img src={p.image} alt=""/> : <Shirt product={p}/>}</div><div className="admin-product-info"><strong>{p.name}</strong><small>{p.color} · {p.material||'Состав не указан'}</small></div><span>{p.variants.reduce((s,v)=>s+v.stock,0)} шт.</span><span>{money(p.price)}</span><i className={p.published?'published':'draft'}>{p.published?'Опубликован':'Скрыт'}</i><div className="admin-actions"><button aria-label={`Редактировать ${p.name}`} onClick={() => {setError('');setSaved('');setEdit(p)}}><Icon name="edit"/></button><button aria-label={`Удалить ${p.name}`} onClick={()=>removeProduct(p)}><Icon name="close"/></button></div></article>)}</div></>:<><header><div><span className="micro">Работа с покупателями</span><h1>Заказы</h1><p>Проверяйте состав, данные доставки и меняйте этап выполнения.</p></div></header>{error&&<p className="auth-error">{error}</p>}<div className="admin-orders">{orders.length?orders.map(o=><article key={o.id}><header><div><span className="micro">{new Date(o.created_at).toLocaleDateString('ru-RU')}</span><h2>Заказ №{o.id}</h2></div><div><strong>{money(Number(o.total))}</strong><select value={o.status} disabled={orderTransitions[o.status].length===1} onChange={e=>status(o,e.target.value)}>{orderStatuses.filter(([value])=>orderTransitions[o.status].includes(value)).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></div></header><div className="admin-order-body"><section><span>Покупатель</span><strong>{o.customer_name}</strong><small>{o.phone}<br/>{o.email}</small></section><section><span>Доставка</span><p>{o.address}</p>{o.comment&&<small>Комментарий: {o.comment}</small>}</section><section className="admin-order-items"><span>Состав заказа</span>{o.items.map(item=><div key={item.id}><p>{item.name}<small>Размер {item.size} · {item.quantity} шт.</small></p><strong>{money(item.price*item.quantity)}</strong></div>)}</section></div></article>):<div className="admin-orders-empty">Новых заказов пока нет</div>}</div></>}
...
function App() {
  ...
  const page = useMemo(() => {
    ...
    if(route.name==='cart') return <Cart cart={cart} change={change} remove={remove} go={go} user={user} onOrdered={()=>{setCart([]);api.products().then(x=>setProducts(x.products)).catch(()=>{})}}/>;
